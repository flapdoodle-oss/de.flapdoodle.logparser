...
public class At extends AbstractStackElement {
	...
	private static final Pattern PATTERN = join(
			...
			compile("\\("),
			group(compile("Native Method"), compile("|"), compile("Unknown Source"), compile("|"), compile("<generated>"),
					compile("|"), namedGroup(FILE, "([a-zA-Z][a-zA-Z0-9]+)\\.java"), compile(":"), namedGroup(LINE_NR, "\\d+")),
			compile("\\)"));
