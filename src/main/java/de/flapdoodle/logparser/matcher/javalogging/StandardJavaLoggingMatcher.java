...
import de.flapdoodle.logparser.matcher.NamedPatterns;
import de.flapdoodle.logparser.stacktrace.StackTrace;

public class StandardJavaLoggingMatcher implements IMatcher {

	NamedPatterns _firstLine = new NamedPatterns(new NamedPattern("^"), new DatePattern(), new NamedPattern(
			"(\\s+)"), new ClassPattern(), new NamedPattern("(\\s+)"), new MethodPattern());

	NamedPattern _levelPattern=new LevelPattern();
	NamedPatterns _secondLine = new NamedPatterns(new NamedPattern("^"),new LevelPattern(),new NamedPattern("message","(?<message>.*)$"));
	@Override
	public boolean match(String firstLine) {
//		int currentPosition = 0;

		return _firstLine.find(firstLine);
...
//		}
	...
	public void process(List<String> lines) throws IOException {
		...
		Optional<Map<String, String>> match = _firstLine.parse(firstLine);
		if (!match.isPresent()) throw new IllegalArgumentException("Could not parse "+firstLine);
		
		if (lines.size()>1) {
			
			String secondLine=lines.get(1);
			Optional<Map<String, String>> secondMatch = _secondLine.parse(secondLine);
